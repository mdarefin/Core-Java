...
public class HeapSort {
	...
	public static void MAX_HEAPIFY(int A[], int i) {
		int l = LEFT(i);// the left element's index which is 2*i+1 (for zero
		int r = RIGHT(i);// right index = 2*i+2;
		int largestElementIndex = -1;// index can't be negative so initialise
										// largest index , it will be used later

		if (l < heapSize && A[l] > A[i]) {
			largestElementIndex = l;
		} else // if max heap property is not violated copy the root's index in
				// largestElementIndex
		{
		...
		if (largestElementIndex != i) {
			...
			// after swap, recursively call the MAX_HEAPIFY on the largest
			// index(root element)
